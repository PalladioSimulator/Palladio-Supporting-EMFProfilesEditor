...
//        subMonitor.worked(1);
    ...
    private void createPluginXml(IProgressMonitor monitor) throws CoreException {
        IFile pluginXmlFile = project.getFile(EMFProfileProjectNature.PLUGIN_XML_FILE_NAME);
        if (!pluginXmlFile.exists()) {
            ...
            pluginXmlContent.append("</plugin>\n"); //$NON-NLS-1$
            InputStream source = new ByteArrayInputStream(pluginXmlContent.toString().getBytes());
    ...
    private void configureBinBuildProperties() throws CoreException {
        ...
        if (!buildPropFile.exists()) {
            ...
            InputStream source = new ByteArrayInputStream(buildPropContent.toString().getBytes());
    ...
    private void createProfile(IProgressMonitor monitor, IProject project) throws IOException {
        profileDiagramResource = EMFProfileDiagramEditorUtil.createDiagram(
                EMFProfileProjectNatureUtil.getDefaultProfileDiagramURI(project), SubMonitor.convert(monitor, 0));
        setProfileDiagramData();
        saveProfileDiagramResource();
    }

    private void saveProfileDiagramResource() throws IOException {
        TransactionalEditingDomain editingDomain = TransactionUtil.getEditingDomain(profileDiagramResource);
    ...
    private Profile getProfileFromResource() {
        if (profileDiagramResource == null)
            return null;
        for (TreeIterator<EObject> contents = profileDiagramResource.getAllContents(); contents.hasNext();) {
    ...
    private void createManifest(String projectPath) {
        ...
        try {
            ...
            writer.write("Bundle-SymbolicName: " + projectData.getProfileNamespace() + "."
                    + projectData.getProfileName() + " ;singleton:=true\n");
            writer.write("Bundle-Version: 1.0.0.qualifier\n");
            writer.write("Bundle-RequiredExecutionEnvironment: JavaSE-1.8\n");
            writer.write(
                    "Require-Bundle: org.modelversioning.emfprofile," + "org.modelversioning.emfprofile.registry\n");
